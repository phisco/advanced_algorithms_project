...
-   [read_peaks.sh](read_peaks.sh) : reads memory peaks from a specified directory of files outputted from massif and
    produces a csv with the measurements, run with ```./read_peaks.sh massif_files_directory```
-   [read_peaks.py](read_peaks.py) : script used by read_peaks.sh, reads res.txt and produces res.csv
### Notebooks :
...
-   [mem_graphs/*](mem_graphs) : graphs generated to be used for memory profiling
-   [res.txt](res.txt), [res.csv](res.csv) : memory usage peaks generated by ```read_peaks.sh``` and used for plotting in
    ```memory.ipynb```
-   [time_optimized_0_1000.txt](time_optimized_0_1000.txt), [time_optimized_0_1000.csv](time_optimized_0_1000.csv) : timeings of the 4 algorithms (tarjan, nuutila,
    pearce, non recursive pearce) generated by ```time_test.sh``` and used for plotting in
    ```time_analysis.ipynb```
-   [tarjan.html](tarjan.html),[nuutila.html](nuutila.html), [pearce.html](pearce.html), [pearceNR.html](pearceNR.html) : the 3d plots of the timing results for tarjan, nuutila, pearce and pearce not recursive in (V, E, t) space